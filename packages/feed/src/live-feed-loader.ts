...
import { FeedError, FeedLoadError, type FeedErrorTypes } from "./feed-errors.js";
		...
		loadCollection: async ({ filter }) => {
			try {
				...
				if (filter) {
					...
					if (filter.category) {
						items = items.filter(item => 
							item.categories?.some(cat => 
								cat.label?.toLowerCase().includes(filter.category!.toLowerCase()) ||
								cat.term?.toLowerCase().includes(filter.category!.toLowerCase())
							)
						);
					}
					...
					if (filter.author) {
						items = items.filter(item =>
							item.authors?.some(author =>
								author.name?.toLowerCase().includes(filter.author!.toLowerCase()) ||
								author.email?.toLowerCase().includes(filter.author!.toLowerCase())
							)
						);
					}
					...
					if (filter.since) {
						items = items.filter(item => 
							item.published && item.published >= filter.since!
						);
					}
					
					if (filter.until) {
						items = items.filter(item =>
							item.published && item.published <= filter.until!
						);
					}
				}
